...
### .small[[스타누르기](https://github.com/mrchypark/apiR)는 컨텐츠 제작자를 춤추게 합니다.]
#### [[pdf버전]](https://github.com/mrchypark/MSC_apiR/blob/master/docs/MSC_apiR.pdf) [[문의하기]](http://pf.kakao.com/_RXANd) [[의견 및 오류 신고]](https://github.com/mrchypark/MSC_apiR/issues/new)


.pull-left[
  .pull-right[
    ![](http://www.imaso.co.kr/wp-content/uploads/2017/11/xmaso_272_90-1.png.pagespeed.ic.uSeG8QUU4f.png)
  ]
]
.pull-right[
.pull-left[
...
.large[머신러닝 프로젝트의 결과물을 서비스에 활용하는 것 또한 많은 노력이 필요함]
...
1. 장애시 전체 서비스의 영향이 적음


단점

1. 개별 서비스 테스트 + 전체 테스트가 필요
1. 서비스간 interface 관리 이슈가 발생
1. 전체 서비스의 추가적인 로그 관리 필요
]]
.pull-right[  
<br>
...
.large[[http 표준][http]으로 .pen-p[요청]을 받아서 처리하여 .pen-p[응답]하는 서버]
...
.large[브라우저에서 주소를 입력하고 엔터를 치는 것!]
...
.large[요청하면 주기로 정해진 데이터(ex> 이미지 파일)를 제공]
...
.large[서버에게 GET 요청이 오면 필요한 데이터를 전달해주기.]
...
## [jug][jug] 패키지를 이용해 R로 api 서버를 만드는 만듬
...
.large[요청 헤더에 body라는 이름으로 값을 보내면 POST이고 없으면 GET]
...
.large[서버에게 POST 요청이 한글 문장을 "sent"라는 이름으로 같이 보내면 띄어쓰기 개수를 세서 줌.]
...
.large[[stringr][stringr]은 [정규표현식][regex]을 활용한 글자 처리를 도와주는 패키지]
...
.large[[httr][httr]로 POST 요청을 해보겠습니다.]
...
.large[python 패키지를 R에서 사용할 수 있게 해주는 패키지]
...
.large[python의 가볍고 유명한 웹 서버 개발 프레임워크]
...
![](https://cdn.namuwikiusercontent.com/storage/3b3c3596c82edba84d54284deb63a0af22d816dad012cafb473aa399aa8282f38fce7e237480e5cda154a654d967319a82f445c74ef93cdd73a6f2019262a6d2adfba211c8c68345065248c20418fd83?e=1515489102&k=PVpdkT0kS3aC7koCxH-VyA)
...
## [체레스터 친구하고 날씨 물어보기](http://pf.kakao.com/_FzLHd)
![](https://github.com/mrchypark/MSC_apiR/blob/master/docs/img/qrcode_350.png?raw=true)

---
class: center, middle, title-slide, inverse

## log는 남기고 싶고, 일반DB는 너무 불편하고.tiny[(돈내야 하고...)]

---
class: center, middle, title-slide, inverse

## G사 클라우드 + k사 p친구 + jug(R) + .yellow[G사 Form(?!?)]
### 서버 + 채팅 플랫폼 + 웹서버 + log용 DB
...
![](https://raw.githubusercontent.com/mrchypark/MSC_apiR/master/docs/img/ssh.png)
...
## [screen](http://dreamlog.tistory.com/470)

screen은 ssh를 닫아도 안에 있는 window session을 유지시켜 줌.

.pull-left[![](http://cfile7.uf.tistory.com/image/224AF148564211851597D0)]
.pull-right[![](http://cfile25.uf.tistory.com/image/2522F049564212CC1DCEB7)]

---
class: center, middle, title-slide, inverse

## 그래도 머신러닝 프로젝트 하는데 서버 만드는 법까지 알아야 하는가?
...
## 저한테 연락하시면 됩니다.
.center[
  [![](https://item.kakaocdn.net/do/d0195d9f27e6eab9548b7ce619b07c54617ea012db208c18f6e83b1a90a7baa7)](http://pf.kakao.com/_RXANd)
]
---
class: center, middle, title-slide, inverse
